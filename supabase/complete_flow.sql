...
-- Run this once in Supabase SQL Editor. Backup first if production data already exists.
...
alter table public.parents add column if not exists cnic_hash text;
alter table public.parents add column if not exists whatsapp text;
...
declare r public.access_requests; begin
 ...
 if p_approve then
   ...
   update public.access_requests set status='rejected',admin_note=p_note,reviewed_by=auth.uid(),reviewed_at=now(),updated_at=now() where id=r.id returning * into r;
 end if;
 insert into public.audit_logs(actor_id,action,entity_type,entity_id,metadata) values(auth.uid(),case when p_approve then 'approve_access_request' else 'reject_access_request' end,'access_request',r.id,jsonb_build_object('student_id',r.student_id));
 return r;
end; $$;
